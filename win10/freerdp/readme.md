**Remote Desktop from Windows 10 guest to Linux host:**

- On the host install `>=net-misc/freerdp-2.3.2` with the USE flags "server openh264" enabled.
- In your running X session run the following command:<br/>
`freerdp-shadow-cli /bind-address:192.168.100.100 -auth -sec-nla` (change to your IP addresses)<br/>
In KDE Plasma I use "System Settings -> Startup and Shutdown -> Autostart" to start and stop the server. You can use my scripts [*/usr/local/bin/freerdp-start.sh*](https://raw.githubusercontent.com/q-g-j/gentoo-stuff/master/usr/local/bin/freerdp-start.sh) and [*/usr/local/bin/freerdp-stop.sh*](https://raw.githubusercontent.com/q-g-j/gentoo-stuff/master/usr/local/bin/freerdp-stop.sh).
Install them as login / logout scripts.
- In the Windows 10 guest download [*wfreerdp.zip*](https://github.com/q-g-j/gentoo-stuff/blob/master/win10/freerdp/wfreerdp.zip?raw=true) and extract to e.g. "C:\Program Files\FreeRDP Client".
- recompiled and updated FreeRDP Client to newest version (3.0.0-dev, x86_64) - now needs `/u:<linux-username>` and `/p:<linux-password>` as parameters but with *-auth* they can be filled with any letters.
- Create a desktop shortcut with the following command:<br/>
`"C:\Program Files\FreeRDP Client\wfreerdp.exe" /v:192.168.100.100 /u:<linux-username> /p:<linux-password> /f /rfx /floatbar:sticky:off,default:hidden,show:fullscreen`<br/><br/>

Unfortunately the Windows version of the client lacks command line output. It can be piped to a file though:<br/>
`wfreerdp.exe /help > %HOMEPATH%\Desktop\freerdp-out.txt 2>&1`<br/><br/>

Here is the output of `wfreerdp.exe /help`:

```
FreeRDP - A Free Remote Desktop Protocol Implementation
See www.freerdp.com for more information

Usage: wfreerdp.exe [file] [options] [/v:<server>[:port]]
...
Examples:
    E:\Incoming\FreeRDP\wfreerdp.exe connection.rdp /p:Pwd123! /f
    E:\Incoming\FreeRDP\wfreerdp.exe /u:CONTOSO\JohnDoe /p:Pwd123! /v:rdp.contoso.com
    E:\Incoming\FreeRDP\wfreerdp.exe /u:JohnDoe /p:Pwd123! /w:1366 /h:768 /v:192.168.1.100:4489
    E:\Incoming\FreeRDP\wfreerdp.exe /u:JohnDoe /p:Pwd123! /vmconnect:C824F53E-95D2-46C6-9A18-23A5BB403532 /v:192.168.1.100
...
For Gateways, the https_proxy environment variable is respected:
    ...
    E:\Incoming\FreeRDP\wfreerdp.exe /g:rdp.contoso.com ...
